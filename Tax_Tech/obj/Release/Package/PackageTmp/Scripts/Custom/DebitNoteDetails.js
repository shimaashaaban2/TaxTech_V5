...
function getDebitNoteLines(docId) {
    sendGetRequest(`/DebitNote/DebitLinesWithActions?docId=${docId}`, function (e) {
...
function getDocByInternalID(event) {
    ...
    sendPostRequest(`DebitNote/GetInvoiceInfo?internalID=${internalID.value}`, null, function (res) {
        event.target.disabled = false;
        if (res.msg == 'success') {
...
function submitLinkedInvoices(event, debitNoteId) {
    event.target.disabled = true;
    ...
    const total = document.querySelector('#totalLinkedInvoicesAmount');
    const debitTotal = document.querySelector('#CredittotalAmount');

    if (parseFloat(total.innerText) > parseFloat(debitTotal.innerText)) {
        swal.fire({
    ...
    sendPostRequest(`/DebitNote/UpdateLinkedInvoices?debitNoteId=${debitNoteId}&total=${total.innerText}&debitTotal=${debitTotal.innerText}`, formData, function (e) {
        document.querySelector('#PopupMsg').innerHTML = e.view;
        event.target.disabled = false;
    });
}
...
function getLinkedInvoices(creditNoteId) {
    sendPostRequest(`/DebitNote/GetLinkedInvoices?docId=${creditNoteId}`, null, function (e) {
        if (e.msg == 'success') {
