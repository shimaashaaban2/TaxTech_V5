window.addEventListener('load', function () {
    getVendorsReportList();
});

function getVendorsReportList() {
    sendGetRequest(`/Reports/VendorsList`, function (e) {
...
function filterByDate() {
    ...
    ShowLoading(2);
    sendGetRequest(`/Reports/FilterVendorsByDate?fromDate=${fromDate.value}&toDate=${toDate.value}&docTypeID=${docTypeID.value}`, function (e) {
        document.querySelector('#list').innerHTML = e;
        HideLoading(2);
        $('#thebasic-datatable').DataTable({ "pageLength": 50 });
    });
...
function exportVendorsTotalsToExecl() {
    const fromDate = document.querySelector('#fromDate');
    const toDate = document.querySelector('#toDate');
    const docTypeID = document.querySelector('#DocumentTypeID');
    const lang = extractLangCookie();
    ...
    iframe.src = url + `Reports/ExportVendorsReport?fromDate=${fromDate.value}&toDate=${toDate.value}&docTypeID=${docTypeID.value}`;
