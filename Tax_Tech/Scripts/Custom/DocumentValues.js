...
function getDocsReportList() {
    sendGetRequest(`/Reports/DocumentsValuesList`, function (e) {
...
function filterByDate() {
    ...
    ShowLoading(2);
    sendGetRequest(`/Reports/FilterDocumentValuesByDate?fromDate=${fromDate.value}&toDate=${toDate.value}&docTypeID=${docTypeID.value}`, function (e) {
        document.querySelector('#list').innerHTML = e;
        HideLoading(2);
    });
}

function exportDocValuesToExecl() {
    const fromDate = document.querySelector('#fromDate');
    const toDate = document.querySelector('#toDate');
    const docTypeID = document.querySelector('#DocumentTypeID');
    const lang = extractLangCookie();
    ...
    iframe.src = url + `Reports/ExportDocValuesReport?fromDate=${fromDate.value}&toDate=${toDate.value}&docTypeID=${docTypeID.value}`;
