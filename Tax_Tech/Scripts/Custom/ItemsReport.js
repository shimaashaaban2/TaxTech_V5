...
function getItemsReportList() {
    sendGetRequest(`/Reports/ItemsList`, function (e) {
...
function filterByDate() {
    ...
    ShowLoading(2);
    sendGetRequest(`/Reports/FilterItemsByDate?fromDate=${fromDate.value}&toDate=${toDate.value}&docTypeID=${docTypeID.value}`, function (e) {
        document.querySelector('#list').innerHTML = e;
        HideLoading(2);
        $('#thebasic-datatable').DataTable({ "pageLength": 50 });
    });
...
function exportItemsTotalsToExecl() {
    const fromDate = document.querySelector('#fromDate');
    const toDate = document.querySelector('#toDate');
    const docTypeID = document.querySelector('#DocumentTypeID');
    const lang = extractLangCookie();
    ...
    iframe.src = url + `Reports/ExportItemsReport?fromDate=${fromDate.value}&toDate=${toDate.value}&docTypeID=${docTypeID.value}`;
