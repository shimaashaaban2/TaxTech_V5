window.addEventListener('load', function () {
    getBranchesReportList();
});

function getBranchesReportList() {
    sendGetRequest(`/Reports/BranchesList`, function (e) {
...
function filterByDate() {
    ...
    ShowLoading(2);
    sendGetRequest(`/Reports/FilterBranchesByDate?fromDate=${fromDate.value}&toDate=${toDate.value}&docTypeID=${docTypeID.value}`, function (e) {
        document.querySelector('#list').innerHTML = e;
        HideLoading(2);
        $('#thebasic-datatable').DataTable({ "pageLength": 50 });
    });
...
function exportBranchesTotalsToExecl() {
    const fromDate = document.querySelector('#fromDate');
    const toDate = document.querySelector('#toDate');
    const docTypeID = document.querySelector('#DocumentTypeID');
    const lang = extractLangCookie();
    ...
    iframe.src = url + `Reports/ExportBranchesReport?fromDate=${fromDate.value}&toDate=${toDate.value}&docTypeID=${docTypeID.value}`;
